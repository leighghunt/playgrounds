 ...
 */
routers.Admin.prototype.send = function(view, options) {
    ...
    this.res.send(Bones.plugin.templates.Admin({
        version: time,
        title: this.pageTitle(main),
        main: $(main.el).html(),
        startup: 'Bones.initialize(function(models, views, routers, templates) {'+
                 'new views.' + main.constructor.title +'('+ o +').attach().render().activeLinks().scrollTop()'+
                 '});'
    }));
};
//
